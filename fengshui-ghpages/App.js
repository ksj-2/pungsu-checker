...
import { analyzeFengShui } from './geminiService.js';
import FengShuiForm from './FengShuiForm.js';
...
const App = () => {
  ...
  const handleAnalyze = useCallback(async () => {
    ...
    try {
      ...
      let userFriendlyMessage = '알 수 없는 오류가 발생했습니다. 잠시 후 다시 시도해주세요.';

      if (e instanceof TypeError && e.message.includes('fetch')) {
        userFriendlyMessage = '인터넷 연결을 확인해주세요. 서버에 접속할 수 없습니다.';
      } else if (e instanceof Error) {
        // In a real app, you might check for specific custom error types
        // For this mock, we can just provide a more generic server-side message
        userFriendlyMessage = 'AI 분석 서버에서 오류가 발생했습니다. 잠시 후 다시 시도해주세요.';
      }
